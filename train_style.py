...
from style_models import Gen as Generator
from style_models import Dis as Discriminator
...
for i in range(start_epoch, n_epochs):
    for j, batch in tqdm(enumerate(dataloader)):
        ...
        rotated_recovered_A = random_rotator(recovered_A.detach()).to(device)
        rotated_recovered_B = random_rotator(recovered_B.detach()).to(device)
        
        loss_ccr_real = (criterion_mse(netD_A(rotated_real_A),netD_A(real_A)) + criterion_mse(netD_B(real_B), netD_B(rotated_real_B)))*lambda_real
        loss_crr_fake = (criterion_mse(netD_B(rotated_fake_B),netD_B(fake_B.detach())) + criterion_mse(netD_A(rotated_fake_A), netD_A(fake_A.detach())))*min(0.5,lambda_fake)
        loss_crr_rec = (criterion_mse(netD_A(rotated_recovered_A),netD_A(recovered_A.detach())) + criterion_mse(netD_B(rotated_recovered_B), netD_B(recovered_B.detach())))*min(0.5,lambda_fake)
        total_ccr_loss = loss_ccr_real+loss_crr_fake+loss_crr_rec
        
        total_ccr_loss.backward()
